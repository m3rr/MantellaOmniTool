...
"Because giving NPCs a soul shouldn't require a degree. 
 But we used one to build this anyway."

================================================================================
                                SO, WHAT IS THIS THING?
================================================================================

Okay listen. You want to talk to Skyrim NPCs. You want them to talk back. 
You saw a video on YouTube and thought "that looks cool" but then you saw the 
installation instructions involved Python, CLI terminals, localhost ports, and 
editing INI files. You almost clicked away.

Don't panic. That is why I built this.

This tool is your EASY BUTTON. It is "Sentient Middleware."
Normally, getting AI to talk to Skyrim is like trying to teach a cat calculus. 
The game is old. The AI is new. They hate each other.
This tool forces them to get along. It assumes your PC is broken, your ports 
are blocked, and your paths are wrong—and it fixes them without asking.

[ WHAT IT DOES FOR YOU ]
1. Finds your game: It uses the Hunter Protocol to scan every drive.
2. Installs the boring stuff: Handles Python libraries locally.
3. Fixes the bugs: Port 5000 mismatch, Icon Caching, Firewall Rules.
4. The Neural Bridge: Forces the AI to open trade menus (`[inventory]`) even 
   if the AI forgets to ask.
...
AI isn't magic. It's math. Heavy, hot, expensive math.

[ A. THE COLD START ]
When you first launch the game, Ollama needs to load the model into VRAM.
- Time: 30 seconds to 5 MINUTES.
- Symptom: You say "Hello", and the game freezes.
- Reality: DO NOT PANIC. Just wait.
...
================================================================================
*Everything below this line is for the nerds.*

[ 1. PROLOGUE: THE PHILOSOPHY ]
The h4 Omni-Tool (v1.0b) is designed as a Single-File Solution.
- No Manual Dependencies: It acts as a portable environment.
- Self-Healing: Checks Skyrim.ini logs on every launch.
- Atomic Operations: Never corrupts config files during crashes.

[ 2. SYSTEM ARCHITECTURE ]
A. THE HUNTER (Scanner):
   Source: core/scanner.py
   Scans Registry, C:\Games, and Mod Organizer instances to find Mantella.

B. THE BRIDGE (Neural Interceptor):
   Source: core/bridge_server.py
   Acts as a Proxy between Skyrim (Port 5001) and Ollama (Port 11434).
   It sniffs traffic for trade keywords ("buy", "sell", "wares") and 
   FORCE INJECTS the `[inventory]` tag if the AI forgets it.

C. THE SURGEON (INI Manager):
   Source: utils/ini_surgeon.py
   Surgically enables `bEnableLogging=1` in Skyrim.ini.
...
2. Wizard: Let the First-Run Wizard scan your inputs.
3. Configure: Click [2] CONFIGURE. Select your AI Model.
4. Activate: Click [3] ACTIVATE SERVICES. Wait for "Ollama Server Running".
5. DONG: Launch Skyrim. Wait at the Main Menu until you hear a "DONG" sound.

[ THE CODEX GIGAS ]
I included the manual INSIDE the app. Click Help -> Codex to open the 
"Codex Gigas," a built-in encyclopedia explaining every error code.
...
Cause: Port Mismatch.
Solution: Go to [2] CONFIGURE and click Save Configuration. This regenerates 
the config with the new "Dual Port" fix (4999/5000).

[ "Ran out of Memory" Popup ]
Cause: Tool + Chrome + Skyrim + AI Model = OOM.
Solution: Close Chrome. It's a hardware limit.
